...
def get_random_colour() -> tuple[str, tuple[int, int, int]]:
    r, g, b = randint(0, 255), randint(0, 255), randint(0, 255)
    return f"#{r:02x}{g:02x}{b:02x}", (r, g, b)


def get_text_colour(rgb: tuple[int, int, int]) -> str:
    r, g, b = rgb
...
def draw_bounding_boxes(image: Image.Image, detections: list[DetectionDTO]) -> Image.Image:
    ...
    for detection in detections:
        ...
        draw.text((x, y - 20), text, fill=get_text_colour(rgb), font=font)
