...
class TestImageService:
    ...
    @pytest.fixture
    def image_service(self, temp_directories):
    ...
    def test_get_folder_map(self, image_service):
        """Test folder mapping logic."""
        folder_map = image_service._get_folder_map()
    ...
    def test_list_images_uploaded_folder(self, image_service, temp_directories):
    ...
    def test_list_images_with_pagination(self, image_service, temp_directories):
        ...
        results = image_service.list_images("uploaded", limit=5, offset=0)
        assert len(results) == 5
        
        results = image_service.list_images("uploaded", limit=5, offset=5)
        assert len(results) == 5

    def test_list_images_all_folders(self, image_service, temp_directories):
    ...
    def test_get_image_by_id_success(self, image_service, temp_directories):
        """Test getting image by ID successfully."""
        image_path = temp_directories["uploaded"] / "test.jpg"
        img = Image.new('RGB', (100, 100), color='red')
        img.save(image_path, format="JPEG")
        
        result = image_service.get_image_by_id("test.jpg", "uploaded")
        
        assert result["filename"] == "test.jpg"
        assert result["format"] == "JPEG"

    def test_get_image_by_id_not_found(self, image_service):
    ...
    def test_delete_image_success(self, image_service, temp_directories):
        """Test successful image deletion."""
        image_path = temp_directories["uploaded"] / "test_delete.jpg"
        img = Image.new('RGB', (100, 100), color='red')
        img.save(image_path, format="JPEG")
        
        result = image_service.delete_image("test_delete.jpg", "uploaded")
    ...
    def test_delete_all_images_single_folder(self, image_service, temp_directories):
    ...
    def test_delete_all_images_all_folders(self, image_service, temp_directories):
    ...
    def test_move_image_success(self, image_service, temp_directories):
        """Test successful image move."""
        source_path = temp_directories["uploaded"] / "test_move.jpg"
        img = Image.new('RGB', (100, 100), color='red')
        img.save(source_path, format="JPEG")
        
        result = image_service.move_image("test_move.jpg", "uploaded", "edited")
    ...
    def test_move_image_not_found(self, image_service):
    ...
    def test_move_image_target_exists(self, image_service, temp_directories):
...
@pytest.mark.unit
class TestImageServiceWithRepository:
    ...
    @pytest.fixture
    def image_service_with_repo(self, temp_directories, mock_repository):
        mock_dir_manager = Mock()
        mock_dir_manager.get_directory.side_effect = lambda folder: temp_directories.get(folder)
    ...
    def test_get_image_path_from_db(self, image_service_with_repo, mock_repository, temp_directories):
        ...
        result = image_service_with_repo.get_image_path("db_test.jpg", "uploaded")
        assert result == image_path
        mock_repository.get_by_filename.assert_called_once_with("db_test.jpg", "uploaded")

    def test_get_image_by_id_from_db(self, image_service_with_repo, mock_repository, temp_directories):
        image_path = temp_directories["uploaded"] / "db_meta.jpg"
        record = self._make_record("db_meta.jpg", "uploaded", image_path)
        mock_repository.get_by_filename.return_value = record

        result = image_service_with_repo.get_image_by_id("db_meta.jpg", "uploaded")
        assert result["filename"] == "db_meta.jpg"
        assert result["path"] == str(image_path)

    def test_delete_image_uses_db_path(self, image_service_with_repo, mock_repository, temp_directories):
        ...
        result = image_service_with_repo.delete_image("db_delete.jpg", "uploaded")

        assert result["status"] == "success"
        assert not image_path.exists()
        mock_repository.delete_by_filename.assert_called_once_with("db_delete.jpg", "uploaded")

    def test_list_images_from_db(self, image_service_with_repo, mock_repository):
        record = self._make_record("listed.jpg", "uploaded", "/tmp/listed.jpg")
        mock_repository.list_by_folder.return_value = [record]

        results = image_service_with_repo.list_images("uploaded", limit=10, offset=0)
    ...
    def test_move_image_updates_db(self, image_service_with_repo, mock_repository, temp_directories):
        ...
        result = image_service_with_repo.move_image("db_move.jpg", "uploaded", "edited")
